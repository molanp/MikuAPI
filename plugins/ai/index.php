...
class ai
{
    ...
    public const profile = '搭载青云客ai与本地词库，相对智能的ai。与ai普普通通的对话吧！或许词库有点二次元？';
    public const method = 'ANY';
    public const author = 'molanp';
    public const request_par = [
        "*msg" => "你要对ai说的话"
    ];
    ...
    public function init($get)
    {
        ...
        if (empty($data)) {
            ...
            if ($result["result"] == 0) {
                ...
                str_ireplace("{br}", "\n", $data);
                if (strpos($data, "提示") !== false) {
                    $data = mb_substr($data, 0, strpos($data, "提示"), "utf-8");
                }
                if (strpos($data, "公众号") !== false) {
                    $data = NULL;
                }
                if (strpos($data, "taobao.com") !== false) {
                    $data = NULL;
                }
                if (strpos($data, "淘宝") !== false) {
                    $data = NULL;
                }
            }
        }
